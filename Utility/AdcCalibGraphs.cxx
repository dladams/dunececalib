...
void tf1Print(TF1* pfit) {
  std::string name = pfit->GetName();
  int npar = pfit->GetNpar();
  cout << name << ": ";
  for ( int ipar=0; ipar<npar; ++ipar ) {
    if ( ipar ) cout << ", ";
    cout << pfit->GetParameter(ipar);
  }
  cout << endl;
}
...
: m_dstName(a_dstName),
  ...
  m_graphChannelEnd(0) {
  const Name myname = "AdcCalibGraphs:ctor: ";
  m_fits["Height"] = {offsetLineShiftedTF1(0, 140, 0,  1, 0, -4, 12, "HeightFit")};
  m_fits["Area"] =   {offsetLineShiftedTF1(0, 900, 0,  1, 0, -4, 12, "AreaFit")};
  m_fits["Area"][0]->SetParLimits(1, 0.0, 10000.0);
  m_fits["Shaping"] = {offsetLinePedTF1(0, 0, 4,  0, 12, "shapingFitPlus"),
                       offsetLinePedTF1(0, 0, 4, -4,  0, "shapingFitMinus")};
  m_fits["ChiSquare"] = {offsetLineShiftedTF1(0, 900, 0,  1, 0, -4, 12, "ChiSquareFit")};
  m_fits["CSNormDof"] = {};
...
AdcCalibGraphs::~AdcCalibGraphs() {
  const Name myname = "AdcCalibGraphs::dtor: ";
...
TF1* AdcCalibGraphs::getFitFunction(Name varName, Index ifun) {
  const Name myname = "AdcCalibGraphs::getFitFunction: ";
  ...
  if ( ifun >= m_fits[varName].size() ) {
    cout << myname << "Invalid index " << ifun << " for variable " << varName << endl;
    return nullptr;
  }
  return m_fits[varName][ifun];
}

//**********************************************************************

int AdcCalibGraphs::setFitFunction(Name varName, TF1* pfit, Index ifun) {
  const Name myname = "AdcCalibGraphs::setFitFunction: ";
  NameVector::const_iterator ivna = find(m_varNames.begin(), m_varNames.end(), varName);
  if ( ivna == m_varNames.end() ) {
    cout << myname << "Unknown variable: " << varName << endl;
    return 1;
...
Index AdcCalibGraphs::makeGraphs(Index icha1, Index ncha) {
  // Make the graphs.
  Index rstat = makeGraphsInternal("", icha1, ncha, m_gras, 1, m_markerType);
...
makeGraphsInternal(Name dstNameIn, Index icha1, Index ncha,
  ...
  const Name myname = "AdcCalibGraphs::makeGraphsInternal: ";
  Index icha2 = icha1 + ncha;
  Name dstName = dstNameIn.size() ? dstNameIn : m_dstName;
  cout << myname << "Making graphs for dataset " << dstName << endl;
  for ( Name varName : m_varNames ) {
    for ( Index icha=icha1; icha<icha2; ++icha ) {
      if ( graph(varName, icha, dstName) != nullptr ) {
        cout << myname << "ERROR: Graph " << varName
             << " for dataset " << dstName << " channel " << icha
             << " already exists." << endl;
        return 1;
      }
    }
  ...
  for ( Name varName : m_varNames ) {
    for ( Index icha=icha1; icha<icha1+ncha; ++icha ) {
      ...
      for ( TF1* pfit : m_fits[varName] ) {
        ...
        if ( xmax > xmin ) {
          std::vector<float> savePars;
          for ( int ipar=0; ipar<pfit->GetNpar(); ++ipar ) {
            savePars.push_back(pfit->GetParameter(ipar));
          }
          pgra->Fit(pfit, fopt.c_str(), "", xmin, xmax);
          for ( Index ipar=0; ipar<savePars.size(); ++ipar ) {
            pfit->SetParameter(ipar, savePars[ipar]);
          }
          fopt = fopt0 + "+";
          ++nfit;
        }
...
PadPtr AdcCalibGraphs::pad(Name graName, Index icha, Index nx, Index ny) {
  ...
  if ( pgraFit->GetListOfFunctions()->GetEntries() ) {
    ...
    if ( varName == "Shaping" ) {
      ...
      if ( haveMinus ) {
        shapingTime = pfit->GetParameter("Pedestal");
        sslab.precision(2);
        sslab << "Minus shaping time: " << shapingTime << " tick";
      ...
      if ( pfit != nullptr ) {
        ...
        sslab.str("");
        sslab.precision(2);
        sslab << "Plus shaping time: " << shapingTime << " tick";
  ...
  if ( graName == "Shaping" ) {
    ...
    if ( shapingTime < 4.1 ) {
      ycen = 2.0;
      dy = 2.0;
    } else if ( shapingTime > 4.7 ) {
      ycen = 0.1*int(10*shapingTime + 0.5);
    }
    ppad->setRangeY(ycen - dy, ycen + dy);
  }
...
GraphPtr AdcCalibGraphs::graph(Name gnam, Index icha, Name dstName) {
  const GraphMapMap* pgras = &m_gras;
  if ( dstName.size() && dstName != m_dstName ) {
    GraphMapMapMap::const_iterator igmm = m_extraGras.find(dstName);
    if ( igmm == m_extraGras.end() ) return GraphPtr();
    pgras = &igmm->second;
  }
  const GraphMapMap& gras = *pgras;
...
GraphPtr AdcCalibGraphs::channelSummaryGraph(Name varName, Name parName, Name fitName0, Name* pgraName) {
  ...
  for ( GraphMap::value_type igrf : grfs ) {
    ...
    if ( ! pgraSum ) {
      ...
      Name yttl = parName;
      if ( parTitle == "Slope" ) parTitle += " [(ADC count)/(DAC step)]";
      pgraSum->GetYaxis()->SetTitle(parTitle.c_str());
    ...
    if ( pfit == nullptr ) {
      ...
      if ( ipar < 0 ) {
        if ( parName == "cs" ) {
          yval = pfit->GetChisquare();
        } else if ( parName == "csdof" ) {
          float ndof = pfit->GetNDF();
          yval = pfit->GetChisquare()/ndof;
        } else {
          cout << myname << "WARNING: Variable " << varName << " channel " << icha << " function "
               << fitName << " parameter " << parName << " is not valid." << endl;
          cout << myname << "Please provide valid parameter name or \"cs\" or \"csdof\"." << endl;
        }
      } else {
...
channelSummaryPad(Name varName, Name parName, Name fitName0,
  ...
  Name myname = "AdcCalibGraphs::channelSummaryPad: ";
  Name fitName = fitName0.size() ? fitName0 : varName + "Fit";
...
drawChannelSummaryPad(Name varName, Name parName, Name fitName0,
                      double ymin, double ymax, Index xw, Index yw) {
  Name myname = "AdcCalibGraphs::drawChannelSummaryPad: ";
